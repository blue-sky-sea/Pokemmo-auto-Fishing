...
if __name__ == '__main__':
    ...
    #next(csv_reader)
    import pandas as pd
    df =  pd.read_csv(sys.argv[3],encoding='utf-8' )#SR.csv
    print(df)
    for i in range(len(df)):
        label_dic[str(i)] = str(df["Name"][i])
    ...
    datas_placeholder = tf.placeholder(tf.float32, [None, 32, 32, 4])#png [None, 32, 32, 4]
    ...
    with tf.Session() as sess:
        ...
        for fpath, predicted_label in zip(fpaths, predicted_labels_val):
            filepath.append('C:/Users/mizukiyuta/Desktop/CNN-Pokemon-recognition-master/' + fpath)
